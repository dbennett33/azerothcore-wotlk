...
void StormwindVaultGrantEndBossWeapons(Creature* boss)
{
    ...
    map->DoForAllPlayers([&](Player* player)
    {
        ...
        uint32 const itemId = StormwindVaultEpicWeaponForClass(player->getClass());
        if (!itemId)
            return;

        if (player->AddItem(itemId, 1))
        {
            LOG_INFO("scripts.stormwindvault", "Blackiron: granted item {} to {}",
                itemId, player->GetName());
            return;
        }

        player->SendItemRetrievalMail(itemId, 1);
        LOG_INFO("scripts.stormwindvault", "Blackiron: mailed item {} to {} (bags full)",
            itemId, player->GetName());
    });
}
