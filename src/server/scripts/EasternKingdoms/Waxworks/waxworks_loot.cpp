...
void WaxworksGrantEndBossWeapons(Creature* boss)
{
    ...
    map->DoForAllPlayers([&](Player* player)
    {
        ...
        uint32 const itemId = WaxworksEpicWeaponForClass(player->getClass());
        if (!itemId)
            return;

        if (player->AddItem(itemId, 1))
        {
            LOG_INFO("scripts.waxworks", "Voss overtime: granted item {} to {}",
                itemId, player->GetName());
            return;
        }

        player->SendItemRetrievalMail(itemId, 1);
        LOG_INFO("scripts.waxworks", "Voss overtime: mailed item {} to {} (bags full)",
            itemId, player->GetName());
    });
}
